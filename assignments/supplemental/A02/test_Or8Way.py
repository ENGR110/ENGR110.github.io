...
@block
def Or8Way(inp_a, out):
    @always_comb
    def foo():
        out[0] = reduce(lambda x,y: x|y, inp_a)
...
# example: test 8-way Or

a, z = get_Nbit_signal(8), get_Nbit_signal(1)
A = ['00000000']

# randomly generate some test strings
for i in range(4):
    A.append(get_random_Nbit_str(8))

# test
test_output = run_test(Or8Way,a,z,A)
